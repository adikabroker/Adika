...
Worked for 2m 39s
ማስታወሻ: ፕሮጀክቱ Streamlit አይደለም (st.session_state / st.chat_input የሉም)። ማስተካከያው በ Telegram Mini App (ui_components.py + api_service.py) ላይ ነው።

የተስተካከለ
Item	Fix
Home banners	2-column compact hero አልተበላሸም
Chat	bindAdvisorChat() + typing row + /api/ai-advisor chat_message
API	Chat ቅርንጫፍ + human persona + 70% purchase cap
Budget bar	ትክክለኛ ብር፦ 70% / 15% / 15%
Cards	በ 70% የግዢ ገደብ ይጣራሉ
Chat flow
ተጠቃሚ መልእክት →
ኦፕሬተሩ መልስ በመጻፍ ላይ ነው... →
API → chat_reply → Advisor መልስ
ሙሉ ui_components.py፦


api_service.py (chat branch)፦


Zip፦


Act as a Senior Python Backend Developer. Fix the Gemini API 404 error and deprecation warning in api_service.py.
Requirements:

Replace import google.generativeai as genai with the new official SDK: from google import genai.
Initialize the client using client = genai.Client(api_key=...).
Update all API calls from genai.GenerativeModel(...) to client.models.generate_content(...).
Ensure the system prompt for "Adika Senior Financial Advisor" (polite human persona in Amharic) remains intact.
Provide the complete refactored api_service.py code block.

Worked for 3m 10s
Gemini SDK ተስተካክሏል — 404/deprecation ለመፍታት።

ለውጦች
አዲስ SDK: from google import genai → client = genai.Client(api_key=...) → client.models.generate_content(...)
Model fallbacks (404 ሲመጣ):
gemini-2.0-flash → gemini-2.0-flash-001 → gemini-1.5-flash → …
_AdikaGeminiModel — የድሮ GenerativeModel ቦታ drop-in (11 ጥሪዎች)
Legacy fallback — google.generativeai ካለ ብቻ
Persona — Adika Senior Financial Advisor / እኔ·እኛ አልተቀየረም
Deploy
...
Worked for 1m 55s
IndentationError ተስተካክሏል

ምክንያቱ፦ ከimport google.generativeai ስንሰርዝ try: ስር የተሳሳተ indent ያለ comment ቀርቶ ነበር፦
...
try:
    # Gemini routed through _AdikaGeminiModel ...
    cprompt = (
py_compile አልፏል። አዲሱን api_service.py deploy ያድርጉ።


Fix the model initialization error in api_service.py:

Change the Gemini model name from gemini-1.5-flash-8b to gemini-1.5-flash.
Ensure proper exception handling and fallback imports for google.genai / google.generativeai.
Output the corrected api_service.py file.

Worked for 38s
ተስተካክሏል

gemini-1.5-flash-8b ተወግዷል
ቅድሚያ: gemini-1.5-flash → gemini-1.5-flash-latest → gemini-2.0-flash …
Fallback: አዲስ google.genai Client፣ ካልሰራ → google.generativeai
config= TypeError ሲመጣ ያለ config እንደገና ይሞክራል
...
Act as a Senior Backend Python Developer. Fix the 404 Model Not Found error in api_service.py by updating the Gemini model configuration to the latest Google API requirements.
Required Modifications in api_service.py:

UPDATE GEMINI MODEL NAMES:
   - Change all primary and fallback model strings from legacy names (gemini-1.5-flash, gemini-2.0-flash, etc.) to:
     Primary Model: gemini-3.6-flash
     Fallback Model: gemini-3.6-pro (or gemini-2.5-flash if applicable)
API CALL & PERSONA HANDLING:
   - Ensure get_gemini_chat_response() uses gemini-3.6-flash.
   - Keep system instructions strict for the Adika Senior Financial Advisor persona (polite Amharic, no mention of AI/bot/LLM, speaking as "እኛ" / "እኔ").
   - Add proper exception handling so if the API fails, it logs a clean error instead of breaking the Streamlit state.
OUTPUT REQUIREMENTS:
   - Provide the 100% complete, uncut code for api_service.py. Do NOT use placeholders.
...
Fallback: gemini-3.6-pro
Then: gemini-2.5-flash → gemini-2.0-flash → gemini-1.5-flash
All explicit model_name= calls now use gemini-3.6-flash.
_gemini_generate / _AdikaGeminiModel try each candidate; failures are logged without crashing the app.
Adika Senior Financial Advisor persona (እኔ/እኛ, no AI/bot) is unchanged.


Fix the SDK Warning in api_service.py regarding Function Calling in generate_content.
Refactor the chat logic in api_service.py to use Google's recommended Chat interface instead of direct generate_content:

IMPLEMENT chats.create & send_message:
   - Replace direct client.models.generate_content(...) calls for chat responses with the recommended Chat session pattern:
     python &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;chat = client.chats.create(model='gemini-2.5-flash', config=config) &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;response = chat.send_message(user_message) &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
   - Ensure system instructions (Adika Senior Financial Advisor persona in polite Amharic) are passed inside the config object during chat creation.
LOGGING & WARNING CLEANUP:
   - This change will eliminate the "Direct use of automatic function calling (AFC) in Models.generate_content is not recommended" warning.
OUTPUT FORMAT:
   - Return the 100% complete, uncut code for api_service.py.

Worked for 1m 5s
AFC warning ተስተካክሏል — chat አሁን የሚመከረው Chat API ይጠቀማል።

ለውጦች
_gemini_chat() አዲስ helper:
chat = client.chats.create(model=model_name, config=config)
response = chat.send_message(user_message)
Advisor chat (chat_message) → _gemini_chat (መጀመሪያ gemini-2.5-flash)
System instruction በ config ውስጥ (Adika Senior Financial Advisor, እኔ/እኛ)
generate_content ላይ AFC ተሰናክሏል (automatic_function_calling: disable)
Chat ካልሰራ → _gemini_generate fallback፤ ስህተት ይሎጋል፣ app አይሰበርም

Act as a Senior Backend Python Engineer. In our project, calling gemini-2.5-flash threw a 404 NOT_FOUND error because of an incorrect model identifier.
Please update api_service.py and ui_components.py with the following fixes:

MODEL NAME CORRECTION & FALLBACK LOGIC:
   - Update the primary Gemini model to gemini-1.5-flash (or gemini-1.5-pro).
   - Implement a fallback try/except block in the API service function:
     * Try calling gemini-1.5-flash first.
     * If it throws an exception/error, automatically fall back to gemini-1.5-pro.
CHAT API SUBMISSION FIX:
   - Ensure the chat submission in ui_components.py properly calls this backend function.
   - Show the Telegram-style "ኦፕሬተሩ መልስ በመጻፍ ላይ ነው..." spinner animation while awaiting response.
   - Ensure the response from Gemini is correctly appended to st.session_state.chat_history and rendered in real-time.
STRICT HUMAN PERSONA:
   - Embed the system prompt requiring the response in polite Amharic as "Adika Senior Financial Advisor" using "እኛ" / "እኔ" and NEVER revealing it is an AI or bot.
Provide the complete code for api_service.py and ui_components.py.

Worked for 36s
ተስተካክሏል

1. Models (api_service.py)
_GEMINI_MODEL_CANDIDATES = (
...
_gemini_generate / _gemini_chat በቅደም ተከተል ይሞክራሉ — 404 ሲመጣ ቀጣዩ model።

2. Chat (ui_components.py — Mini App, አይደለም Streamlit)
POST /api/ai-advisor + chat_message
Spinner: ኦፕሬተሩ መልስ በመጻፍ ላይ ነው...
መልስ በ chat log ላይ ይታያል
3. Persona
Adika Senior Financial Advisor — እኔ/እኛ፣ AI/bot አይገለጽም።
...
Worked for 55s
ሁሉም ፋይሎች እዚህ py_compile አልፈዋል — የአካባቢ api_service.py ላይ IndentationError የለም።

ብዙ ጊዜ ይህ ስህተት የሚመጣው፦

ከፊል paste (አንድ ቁራጭ ብቻ መተካት)
የድሮ deploy ላይ ያለ የተሰበረ ፋይል
ማድረግ ያለብዎት
ሙሉ api_service.py እንደገና upload/deploy ያድርጉ (ከፊል አይተኩ)።


ከ deploy በኋላ ስህተቱ ከቀጠለ ሙሉ traceback (የፋይል ስም + የመስመር ቁጥር) ይላኩ።

I got an "IndentationError: unexpected indent" in ui_components.py.
Please clean up the code indentation completely. Ensure proper Python 4-space indentation throughout the entire file and output the clean, fully working code for ui_components.py.


Worked for 1m 12s
ui_components.py ተጽዷል — ሁሉም top-level Python 0 indent፣ ይዘቱ በ r"""...""" ውስጥ ብቻ ነው።

SELLER_FORM_HTML = r""" ... """
BUYER_FORM_HTML  = r""" ... """
EXPLORER_HTML    = r""" ... """
ast.parse + py_compile አልፈዋል።

አስፈላጊ: ሙሉ ፋይሉን እንደ አንድ ክፍል deploy ያድርጉ — ከፊል paste IndentationError ያስከትላል።
...
Worked for 1m 7s
IndentationError መፍትሄ

ትልቁ HTML በPython string ውስጥ ስለነበር አርትዖት/paste ሲበላሽ IndentationError ይፈጥር ነበር። አሁን ተከፋፍሏል፦

አዲስ መዋቅር
adika_bot/
  ui_components.py          ← አጭር loader ብቻ (~15 መስመር)
  templates/
    seller_form.html
    buyer_form.html
    explorer.html
ui_components.py (ሙሉ)
# ui_components.py — load HTML templates (no giant indented strings)
"""UI template constants for Adika Telegram Mini App."""
...
EXPLORER_HTML = _load("explorer.html")
Deploy
ሙሉ zip ይጭኑ — templates/ folder ጋር መሆን አለበት።


I got an "IndentationError: unexpected indent" in ui_components.py api_service.py.
Please clean up the code indentation completely. Ensure standard Python 4-space indentation throughout the entire file and output the 100% complete, fully working code block without any syntax or indentation errors.

Request was interrupted by the user.

I got an "IndentationError: unexpected indent" in ui_components.py api_service.py. Please clean up the code indentation completely. Ensure standard Python 4-space indentation throughout the entire file and output the 100% complete, fully working code block without any syntax or indentation errors.


Worked for 53s
ሁለቱም ፋይሎች እዚህ ላይ ምንም IndentationError የላቸውም (py_compile አልፏል)።

ui_components.py (ሙሉ — 15 መስመር ብቻ)
# ui_components.py
from pathlib import Path
...
adika_bot/
  ...
  templates/
    ...
    explorer.html
  ...
templates/ ከሌለ ወይም ከፊል api_service.py ብቻ ከተለጠፈ ስህተት ይቀጥላል።
...
ስህተቱ ከቀጠለ ሙሉ traceback ይላኩ (ፋይል + የመስመር ቁጥር)።
        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "Answer warm, conversationally, and directly in Amharic. "
                    "Only give budget breakdowns if explicitly requested."
                )
            }
        ]
       
        # የቀደሙ የውይይት ታሪኮችን መጨመር
...
Worked for 2m 18s
Groq ተገጥሟል — advisor chat አሁን Groqን ይመርጣል።

አዲስ logic
get_groq_response() — የሰጡት pattern (system + history + user)
_advisor_chat_reply()
መጀመሪያ Groq (GROQ_API_KEY)
ካልሰራ → Gemini
Models: llama-3.3-70b-versatile → llama-3.1-70b-versatile → …
Persona
Adika Senior Financial Advisor — ሞቅ ያለ አማርኛ፣ እኔ/እኛ፣ AI/bot አይገለጽም።
...
response = groq_client.chat.completions.create(
    model="openai/gpt-oss-120b", # ወይም "openai/gpt-oss-20b"
    messages=messages,
    temperature=0.7,
    max_tokens=1024,
)
...
get_groq_response() በቅደም ተከተል ይሞክራል — 120b ካልሰራ 20b፣ ከዚያ Llama።
...
def generate_ai_response(prompt, chat_history=[]):
    """
    በመጀመሪያ በ Groq (Llama 3.1) ይሞክራል፤
    Groq ካልሰራ ወደ Gemini Fallback ያደርጋል።
    """
    system_instruction = (
        "You are Adika's Senior Financial & Vehicle Advisor in Ethiopia. "
    ...
    if groq_client:
        try:
            ...
            print(f"Groq failed, switching to Gemini fallback: {e}")
    # 2. Groq ካልሰራ ቀደም ሲል የሰራነውን Gemini Fallback መጥራት
    return gemini_generate(prompt, system=system_instruction)


Worked for 1m 9s
ተገጥሟል — የሰጡት flow አሁን በapi_service.py ውስጥ ነው።

generate_ai_response()
Groq (ፈጣን) → llama-3.1-70b-versatile መጀመሪያ
ካልሰራ → Gemini
ሁለቱም ካልሰሩ → ደህንነቱ የተጠበቀ አማርኛ መልስ
Models
...
# optional fallback:
GEMINI_API_KEY=...
/api/ai-advisor chat → _advisor_chat_reply() → generate_ai_response()።
...
# አዲሶቹ የ Groq ሞዴሎች በቅደም ተከተል

GROQ_MODELS = [
    "llama-3.3-70b-versatile", # አዲሱ እና ትክክለኛው 70B ሞዴል
    "llama3-70b-8192", # ተጠባባቂ (Fallback 1)
    "llama3-8b-8192" # ተጠባባቂ (Fallback 2)
]
def generate_ai_response(prompt, chat_history=[]):
    system_instruction = (
        "You are Adika's Senior Financial & Vehicle Advisor in Ethiopia. "
    ...
    if groq_client:
        ...
        for msg in chat_history:
            messages.append(msg)
        messages.append({"role": "user", "content": prompt})
        # ሞዴሎቹን በቅደም ተከተል ይሞክራል (400/404 error እንዳይመጣ)
        for model_name in GROQ_MODELS:
            try:
                response = groq_client.chat.completions.create(
                    ...
                    temperature=0.6,
                    max_tokens=800,
                    presence_penalty=0.5 # ቃላት እንዳይደጋገሙ ይከላል
                )
                return response.choices[0].message.content
            except Exception as e:
                print(f"Groq model {model_name} failed: {e}")
    # Groq ሙሉ በሙሉ ካልሰራ ወደ Gemini Fallback ያደርጋል
    return gemini_generate(prompt, system=system_instruction)


Worked for 2m 15s
ተዘምኗል — የሰጡት Groq cascade ተገጥሟል።
...
temperature=0.6
max_tokens=800
presence_penalty=0.5 (ድግግሞሽ ለመቀነስ)
Key: config.GROQ_API_KEY
Flow
Groq cascade → Gemini → ደህንነቱ የተጠበቀ አማርኛ መልስ
...
# አሁን የሚሰሩት የ Groq ሞዴሎች

GROQ_MODELS = [
    "llama-3.1-8b-instant", # እጅግ ፈጣን እና አሁን የሚሰራ
    "mixtral-8x7b-32768" # ተጠባባቂ (Fallback)
]
def generate_ai_response(prompt, chat_history=[]):
    system_instruction = (
        "You are Adika's Senior Financial & Vehicle Advisor in Ethiopia. "
...
_GROQ_MODELS = (
    "llama-3.1-8b-instant",  # primary — ፈጣን
    "mixtral-8x7b-32768",    # fallback
)
ያልተቀየረ፦ temperature=0.6, max_tokens=800, presence_penalty=0.5, Groq → Gemini fallback።

Render ላይ GROQ_API_KEY መኖሩን ያረጋግጡ፣ ከዚያ redeploy።
...
Worked for 25s
ይህ configuration አስቀድሞ ተገጥሟል ነው።

አሁን ያለው (api_service.py)
_GROQ_MODELS = (
...
max_tokens	800
presence_penalty	0.5
Flow	Groq → Gemini → Amharic error
Render checklist
GROQ_API_KEY በ Environment Variables
pip install groq / requirements.txt ውስጥ groq>=0.9.0
Full redeploy (ከፊል ፋይል አይተኩ)
አዲስ model ስም ወይም ስህተት (log) ካለ ይላኩ።
...
def generate_ai_response(prompt, chat_history=[]):
    ...
    try:
        response = openrouter_client.chat.completions.create(
            model="openai/gpt-4o-mini", # ፈጣን እና ርካሽ የሆነ ለአማርኛ ምርጥ ሞዴል
            messages=messages,
            temperature=0.3,           # የፈጠራ መጠንን በመቀነስ ትክክለኛ እና ያልተዛባ ቋንቋ እንዲጠቀም ያደርጋል
            frequency_penalty=0.6,     # ቃላትን ደጋግሞ ከመጠቀም በጥብቅ ይከለክላል
            presence_penalty=0.5,      # አላስፈላጊ ማብራሪያዎችን ይቀንሳል
            max_tokens=600
        )
        return response.choices[0].message.content
    except Exception as e:
...
Worked for 2m 13s
OpenRouter ተገጥሟል — አሁን ቀዳሚ AI backend ነው።

Flow
OpenRouter → openai/gpt-4o-mini
ካልሰራ → Groq (llama-3.1-8b-instant)
ካልሰራ → Gemini
Settings (የሰጡት)
Param	Value
...
# OpenRouter Client ብቻ ማዋቀር (ሌሎች አላስፈላጊ SDKዎች ሙሉ በሙሉ ጠፍተዋል)
...
def generate_ai_response(prompt, chat_history=[]):
    """
    OpenRouter (openai/gpt-4o-mini) በመጠቀም ለአዲካ የተጠቃሚ ጥያቄዎችን
    በጥራት፣ ያለ መደጋገም እና በአማርኛ የሚመልስ ዋና ተግባር።
    """
    system_instruction = (
        "You are Adika's Senior Financial & Vehicle Advisor in Ethiopia. "
        "You must respond ONLY in clear, natural, and grammatically correct Amharic. "
...
Worked for 7m 56s
OpenRouter ብቻ — Groq/Gemini ከ chat path ተወግደዋል።

generate_ai_response()
OpenRouter → openai/gpt-4o-mini
temperature=0.3 | frequency_penalty=0.6 | presence_penalty=0.5 | max_tokens=600
Render
OPENROUTER_API_KEY=sk-or-v1-...
pip install openai>=1.40.0
Key ከሌለ፦ ይቅርታ፣ የ AI አገልግሎት አልተዋቀረም።
API ስህተት፦ ይቅርታ፣ አሁን መልስ ማዘጋጀት አልተቻለም...


import os

# ሁሉም የሚያስፈልጉ Config Variables

ADMIN_CHAT_ID_INT = int(os.environ.get("ADMIN_CHAT_ID", 0))
DATABASE_URL = os.environ.get("DATABASE_URL")
WEBAPP_URL = os.environ.get("WEBAPP_URL")

# የድሮዎቹን ስህተት እንዳይፈጥሩ እንደ አስፈላጊነቱ ማቆየት ይቻላል
...
Worked for 45s
config.py አስቀድሞ ዝግጁ ነው — የሰጡት keys ሁሉ አሉ።

GROQ_API_KEY = (os.environ.get("GROQ_API_KEY") or "").strip()
GEMINI_API_KEY = (os.environ.get("GEMINI_API_KEY") or "").strip()
OPENROUTER_API_KEY = (os.environ.get("OPENROUTER_API_KEY") or "").strip()  # ዋናው

ADMIN_CHAT_ID_INT = int(...)   # from ADMIN_CHAT_ID
DATABASE_URL = ...
WEBAPP_URL = ...               # Render hostname ወይም env
Render Environment
Variable	Required
OPENROUTER_API_KEY	አዎ (AI chat)
BOT_TOKEN	አዎ
DATABASE_URL	አዎ
ADMIN_CHAT_ID	ይመከራል
GROQ_API_KEY / GEMINI_API_KEY	አይ (አሁን አይጠቀሙም)
Key ካልተቀመጠ chat ይመልሳል፦
ይቅርታ፣ የ AI አገልግሎት አልተዋቀረም።

ሌላ ማስተካከል ከፈለጉ ይንገሩ።

config.py
Api_service.py ሙሉ ላክልኝ የተስተካከለ
...
config.py
OPENROUTER_API_KEY (ዋና)
GROQ_API_KEY / GEMINI_API_KEY (ተጠባባቂ)
DATABASE_URL, WEBAPP_URL, ADMIN_CHAT_ID_INT, …
api_service.py
OpenRouter ብቻ → openai/gpt-4o-mini
generate_ai_response() + _advisor_chat_reply()
temperature 0.3, frequency_penalty 0.6, presence_penalty 0.5

Render: OPENROUTER_API_KEY=sk-or-v1-... እና pip install openai>=1.40.0 ከዚያ full redeploy።
...
def generate_ai_response(prompt, chat_history=[]):
    """
    OpenRouter API በመጠቀም (እንደ google/gemini-2.0-flash-001 ያሉ ሞዴሎችን)
    ጥያቄዎችን በአማርኛ የሚመልስ ተግባር።
    """
    api_key = config.OPENROUTER_API_KEY
    model_name = getattr(config, "OPENROUTER_MODEL", "google/gemini-2.0-flash-001")
    ...
    headers = {
        ...
        "HTTP-Referer": "https://github.com", # OpenRouter እንዲቀበለው አማራጭ
        "X-Title": "Adika Bot"
    }
...
Worked for 1m 30s
ተዘምኗል — OpenRouter አሁን በrequests (ያለ OpenAI SDK) ይሰራል።

Default model
google/gemini-2.0-flash-001
(በOPENROUTER_MODEL env መቀየር ይቻላል)
...
requests አስቀድሞ በrequirements.txt አለ።
...
def generate_ai_response(prompt, chat_history=[]):
    """
    OpenRouter Free Tier ሞዴሎችን በመጠቀም ጥያቄዎችን
    በጥራት እና በአማርኛ የሚመልስ ዋና ተግባር።
    """
    api_key = config.OPENROUTER_API_KEY
    model_name = getattr(config, "OPENROUTER_MODEL", "meta-llama/llama-3.3-70b-instruct:free")
    api_url = "https://openrouter.ai/api/v1/chat/completions"
...
Worked for 20s
Sandbox አቅም ስለሌለው እዚህ ላይ ቀጥታ የተስተካከለውን ኮድ እሰጣለሁ።

config.py — ይህን ጨምሩ/ቀይሩ
import os
...
api_service.py — generate_ai_response ብቻ ይህን ይተኩ
def generate_ai_response(prompt, chat_history=None, system=None, temperature=0.3):
    """
    OpenRouter Free Tier — default: meta-llama/llama-3.3-70b-instruct:free
    """
...
# optional:
OPENROUTER_MODEL=google/gemini-2.0-flash-001
openai SDK አያስፈልግም — requests ብቻ በቂ ነው (አስቀድሞ በrequirements.txt አለ)።

የተስተካከልውን ብፋይል ላክልኝ


Worked for 1m 47s
የተስተካከሉት ሙሉ ፋይሎች እነዚህ ናቸው፦


ማጠቃለያ

config.py — OPENROUTER_API_KEY + OPENROUTER_MODEL
api_service.py — OpenRouter በrequests (default: google/gemini-2.0-flash-001)
Render
...
def generate_ai_response(prompt, chat_history=[]):
    """
    Groq API በመጠቀም ከአላስፈላጊ ድግግሞሽ ነፃ የሆነ
    የአማርኛ ምላሽ የሚሰጥ ተግባር።
    """
    ...
    system_instruction = (
        ...
        "1. Answer ONLY what the user explicitly asks. Never add unrequested options, lists, or extra advice. "
        "2. DO NOT REPEAT ANY WORD, PHRASE, OR SENTENCE. Each sentence must be unique. "
        "3. Do not invent facts, numbers, or features. If you lack information, state it directly in one sentence. "
        "4. Maximum response length is 3 sentences. Be extremely concise, direct, and professional."
    )
    messages = [{"role": "system", "content": system_instruction}]
    for msg in chat_history:
        messages.append(msg)
    ...
    try:
        completion = client.chat.completions.create(
            ...
            messages=messages,
            temperature=0.1, # ድግግሞሽን እና የፈጠራ ወሬን ለመግታት ዝቅ ተደርጓል
            max_tokens=400,
            top_p=0.8
        )
        ...
        print(f"Groq API Error: {e}")
        return "ይቅርታ፣ አገልግሎቱን ማቅረብ አልተቻለም። እባክዎ ትንሽ ቆይተው እንደገና ይሞክሩ።"


Worked for 58s
Groq ተገጥሟል — አሁን ዋና AI backend ነው።

Settings
Param	Value
Model	llama-3.1-8b-instant (via GROQ_MODEL)
temperature	0.1
max_tokens	400
top_p	0.8
Render
...
from config import logger, MAX_IMAGE_BYTES, ADMIN_CHAT_ID_INT, DATABASE_URL, WEBAPP_URL, OPENROUTER_API_KEY, OPENROUTER_MODEL, GROQ_API_KEY, GROQ_MODEL
from models import (
    LAST_DB_ERROR,
...
# ---------------------------------------------------------------------------
# Groq API (primary chat) — concise Amharic advisor
# ---------------------------------------------------------------------------
_ADIKA_SYSTEM = (
    "You are Adika's Senior Financial & Vehicle Advisor in Ethiopia. "
    "You must respond ONLY in clear, natural, and grammatically correct Amharic. "
...
def generate_ai_response(prompt, chat_history=None, system=None, temperature=0.1):
    """
    Groq API — short Amharic replies without redundant repetition.
    """
    ...
    try:
        model_name = (
            (GROQ_MODEL if GROQ_MODEL else None)
...
def get_openrouter_response(prompt, chat_history=None, system=None, temperature=0.1, max_tokens=400):
    """Compatibility alias — routes to Groq generate_ai_response."""
    return generate_ai_response(
        prompt, chat_history=chat_history, system=system, temperature=temperature
    )


def _advisor_chat_reply(user_message, *, system=None, temperature=0.1):
    return generate_ai_response(
        user_message,
        chat_history=[],
        system=system,
