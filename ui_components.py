...
(function () {
  ...
  $("advGen").onclick = function () {
    var budget = Number($("advBudget").value || 0);
    var income = Number($("advIncome").value || 0);
    var down = Math.round(budget * 0.3);
    var loan = Math.max(budget - down, 0);
    var monthly = income ? Math.round(loan * 0.018 / 12 + loan / 60) : 0;
    $("advResults").innerHTML =
      '<div class="rounded-xl p-2 bg-white/10"><b>🚗 Automotive</b><p>በጀት ' + money(budget) + ' · ቅድመ ' + money(down) + '</p><button type="button" class="js-live mt-1 text-cyan-300">ጥልቅ የፋይናንስ ትንተና ከ Adika Digital Advisor Live Chat ያድርጉ →</button></div>' +
      '<div class="rounded-xl p-2 bg-white/10"><b>🏠 Real Estate</b><p>ቅድመ ክፍያ አማራጭ ' + money(down) + '</p><button type="button" class="js-live mt-1 text-cyan-300">ጥልቅ የፋይናንስ ትንተና ከ Adika Digital Advisor Live Chat ያድርጉ →</button></div>' +
      '<div class="rounded-xl p-2 bg-white/10"><b>💼 Business ROI</b><p>ዓመታዊ ግምት 15%–35% በገበያ መስፈርት</p><button type="button" class="js-live mt-1 text-cyan-300">ጥልቅ የፋይናንስ ትንተና ከ Adika Digital Advisor Live Chat ያድርጉ →</button></div>';
