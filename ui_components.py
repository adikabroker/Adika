...
<body class="bg-[#b5eff3] min-h-screen">
  ...
  (function () {
    ...
    function renderAnalysisDashboard(d, budget) {
      var advice = (d && d.advisor_report) ? d.advisor_report : (d && d.advice ? d.advice : (d || {}));
      var body = d.response || d.message || d.chat_reply || advice.expert_advice_amharic || advice.summary_amharic || advice.advice_amharic || advice.advice_am || advice.message || advice.summary || "";
      body = String(body).replace(/\bAI\b/gi, "እኛ").replace(/language model/gi, "እኛ").replace(/\bbot\b/gi, "እኛ");
      var title = advice.verdict_title_amharic || advice.title || advice.summary_title || "የአዲካ ምክር";
      ...
      if (log && !log.dataset.seeded) {
        log.innerHTML = "";
        advisorChatHistory = [];
        var initMsg = "ሰላም! እኔ የ Adika አማካሪ ነኝ። ጥያቄዎን በቀጥታ ይጻፉልኝ — በተፈጥሯዊ አማርኛ እመልሳለሁ።";
        appendAdvisorChat("advisor", initMsg);
        advisorChatHistory.push({ role: "assistant", content: initMsg });
    ...
    (function(){
      function bindAdvisorChat() {
        ...
        function sendChat() {
          ...
          input.value = "";
          showTyping();
          var historyForApi = advisorChatHistory.map(function(h) {
            return {
              role: (h.role === "advisor" || h.role === "assistant") ? "assistant" : "user",
          ...
          .then(function(d){
            removeTyping();
            var msg = (d && (d.response || d.message || d.chat_reply || d.reply)) || "መልስ አልተገኘም።";
            ...
            advisorChatHistory.push({ role: "assistant", content: msg });
          })
          .catch(function(){
            removeTyping();
...
try:
    from api_service import generate_advisor_response  # OpenRouter-backed
except Exception:
