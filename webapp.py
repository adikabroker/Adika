    ...
    <div class="w-full max-w-md bg-white rounded-t-3xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
      ...
      <div class="p-4 overflow-y-auto space-y-3.5 flex-1 text-xs bg-[#f8fafc]">
        <div class="p-2.5 rounded-2xl bg-white border border-[#16acbd]/25 shadow-xs flex items-start gap-2.5 text-[11px] text-slate-600">
          ...
          <div class="leading-snug">የውክልና ሰነዱን ፎቶ ይጫኑ። ዲጂታል QR ካለ በአዲካ ሲስተም ይነበባል። QR የሌለው የቆየ ሰነድ በአካል ቢሮ ማረጋገጥ ያስፈልጋል።</div>
        </div>
        ...
        <div class="p-3 bg-white rounded-2xl border border-slate-200 shadow-xs space-y-2">
          ...
          <p class="text-[10px] text-slate-400 leading-snug">QR ኮዱን በግልጽ አድርገው ይጭኑ። ማረጋገጫው በመሣሪያዎ ላይ ብቻ ይከናወናል።</p>
        </div>
    ...
    function showPoaStateA(url, resEl) {
      ...
      resEl.innerHTML =
        '<div class="p-3.5 rounded-2xl bg-white border border-emerald-200 shadow-sm space-y-3 text-xs">' +
          '<div class="font-black text-emerald-800 leading-snug">✅ የውክልና ሰነዱ ዲጂታል ማረጋገጫ በስኬት ተነቧል!</div>' +
    ...
    function showPoaStateB(resEl) {
      ...
      resEl.innerHTML =
        '<div class="p-3.5 rounded-2xl bg-amber-50 border border-amber-200 text-amber-950 text-xs leading-relaxed">' +
          '⚠️ ይህ ሰነድ በዲጂታል መንገድ ሊጣራ የሚችል QR ኮድ የለውም። የቆየ የውክልና ሰነድ በመሆኑ፣ እባክዎን በአካል በአቅራቢያዎ በሚገኝ የሰነዶች ማረጋገጫና ምዝገባ (ውልና ማስረጃ) ቢሮ በመሄድ ያጣሩ።' +
        '</div>';
    }
          ...
          try {
            ...
            if (code && code.data) {
              ...
              if (/^https?:\/\//i.test(payload)) {
                if (resEl) showPoaStateA(payload, resEl);
              } else {
                // Non-URL QR payload — treat as non-openable digital token; still success read
                // Prefer opening only http(s); otherwise show state B guidance without raw text dump
